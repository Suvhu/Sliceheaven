...
export default function Aforgotpassword(props) {
  ...
  const userValid = async () => {
    const response = await fetch(`http://localhost:5000/api/authadmin/forgotpassword/${id}/${token}`, {
      method: "GET",
      headers: {
  ...
  const sendpassword = async(e)=>{
    e.preventDefault();
    const response = await fetch(`http://localhost:5000/api/authadmin/${id}/${token}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
